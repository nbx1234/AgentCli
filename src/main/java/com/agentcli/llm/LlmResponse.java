...
/**
 * 模型返回结果：普通文本内容 + （可选）工具调用列表 + （可选）推理内容。
 *
 * @param content   回复文本（无文本时可能为空串）
 * @param toolCalls 模型发起的工具调用；无则为空列表
 * @param reasoning 推理内容（DeepSeek reasoning_content）；无则 null
 */
public record LlmResponse(String content, List<ToolCall> toolCalls, String reasoning) {
